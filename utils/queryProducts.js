class QueryProducts {
    ...
    categoryQuery = () => {
        this.products = this.query.category ? this.products.filter(
    ...
    ratingQuery = () => {
        this.products = this.query.rating ? this.products.filter(
            c => parseInt(this.query.rating) <= c.rating && c.rating < parseInt(this.query.rating)
        + 1) : this.products
    ...
    searchQuery = () => {
        this.products = this.query.searchValue ? this.products.filter(
            p => p.name.toLowerCase().includes(this.query.searchValue.toUpperCase())
        ) : this.products
        return this
    }
}
